...
|---------------|-------------|-----------|
|multipart-config-message|yes|signifies a large payload that is multi-part and should always have a value of yes|
...
<li> Chunk the data from step 1 into 125KB parts </li>
<li> Form a post payload from each chunk and apply the necessary standard and custom properties appropriatly </li>
<li> Execute Method Call on Azure IoT Edge </li>
<li> Repeat steps 3 and 4 until all the chunks have been sent </li>
</ol>

Once the final chunk has been executed you can optionally submit a confirmation payload to IoT Edge indicating that a large payload has been sent
...
<li> Count the number of entries cached to see if they match the number in the custom property 'maxPart'.  If they do then process the set as shown in the next section.</li>
</ol>
...
3. Create free or standard IoT hub using [Azure portal](https://ms.portal.azure.com/)
4. Note the iothub connection string. You'll be using it executing Method calls
5. In the [Azure portal](https://ms.portal.azure.com/), navigate to your IoT hub
6. Select IoT Edge from the navigation menu
7. Select Add an IoT Edge device
8. Click on created IoT Edge device then click on "Set modules" tab
9. On Set modules on device page +Add IoT Edge Module you created in step #2
10. Click on "Container Create Option" and paste in the following json
    ```json
    {
        "HostConfig": {
            "Mounts":[
    ...
    ```
11. Click on Routes tab and add the following routes:
    ```json
    "routes": {
        "file_to_hub": "FROM /* INTO $upstream"
    }
    ```
...
## Option 2: Using C2D message (IoT Edge Gateway only)
### From cloud side
...
12. [Create and provision a single Linux IoT Edge device](edgegatewayvm.md)
13. [Build and execute script](https://github.com/iot-for-all/iotedge-gateway-file-upload-c2d/blob/master/app/README-C2D.md) to send the files
